...
def parse_player_history(
    ...
    for item in value.get("GamedayWiseStats", []):
        ...
        price_change = player_value - old_player_value
...
def fetch_public_data(
    ...
    if not skip_history:
        ...
        for asset in assets:
            ...
            asset_scores, asset_breakdown, asset_gamedays = parse_player_history(
                asset, detail_feed
            )
            score_rows.extend(asset_scores)
            breakdown_rows.extend(asset_breakdown)
            gameday_rows.update(asset_gamedays)
            history_count += 1

        write_csv(out_dir / "gameday_scores.csv", GAMEDAY_SCORE_COLUMNS, score_rows)
        write_csv(out_dir / "gameday_score_breakdown.csv", BREAKDOWN_COLUMNS, breakdown_rows)
        write_csv(
            out_dir / "assets_state_snapshot.csv",
            ASSET_STATE_COLUMNS,
            asset_state_snapshot_rows(assets, score_rows),
        )
        write_csv(
            out_dir / "gamedays.csv",
            GAMEDAY_COLUMNS,
            sorted(
                gameday_rows.values(),
                key=lambda row: int(row["gameday_id"]) if row["gameday_id"].isdigit() else 0,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.skip_history:
        ...
        print(f"Wrote gameday history for {history_count} assets.")
    return 0
